...
public class excelDriven {
	...
	public ArrayList<String> getData(String testcaseName, String coloumnName) throws IOException
	{
	...
	FileInputStream fis=new FileInputStream(System.getProperty("user.dir")+"\\src\\test\\java\\resources\\Employee details1.xlsx");
	...
	{
	if(workbook.getSheetName(i).equalsIgnoreCase("ProfileData"))
	{
	...
	if(r.getCell(coloumn)!=null && r.getCell(coloumn).getStringCellValue().equalsIgnoreCase(testcaseName))
	{
	...
	{
	Cell c= cv.next();
	//System.out.println(cv.next().getStringCellValue()); ///to pring al the row cell values
